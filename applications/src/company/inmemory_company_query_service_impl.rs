...
mod tests {
    ...
    async fn find_by_name() -> anyhow::Result<()> {
        ...
        assert!(service.find(command).await?.pop().unwrap().name == "いすゞ自動車".to_string());
    ...
    async fn find_notexist_id_return_err() -> anyhow::Result<()> {
        let service = setup();
        let Err(CompanyQueryError::CompanyNotFound(_)) = service.find_by_id("not exits id".to_string()).await else {
    ...
    async fn pagenation() -> anyhow::Result<()> {
        ...
        assert!(found[0].stock_id == StockId::new("2222".to_string()));
    ...
    async fn find_list() -> anyhow::Result<()> {
        ...
        let result_id_list: Vec<String> = result.into_iter().map(|c| c.stock_id.to_string()).collect();
